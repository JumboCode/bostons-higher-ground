...
export default function CreatePasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const passwordRes = await fetch("/api/onboarding/set-password", {
                method: "POST",
                headers: {
            ...
            if (!passwordRes.ok) {
                ...
                setError(
                    data.error || "Could not set password. Please try again."
                );
                setIsSubmitting(false);
                return;
            }

            const completionRes = await fetch("/api/onboarding/complete", {
                method: "POST",
            });

            if (!completionRes.ok) {
                const data = await completionRes.json().catch(() => ({}));
                setError(
                    data.error ||
            ...
            <div className="w-md px-8 py-8 rounded-3xl bg-white shadow-[0px_24px_50px_0px_rgba(167,74,91,0.2)] flex flex-col items-center space-y-6">
                ...
                >
                    ...
                    <div>
                        <label className="block text-[#555] font-medium">
                            Confirm Password{" "}
                            <span className="text-bhg-error">*</span>
                        </label>
                        <div className="relative mt-1">
                            <input
