...
export function NotifCard({
  ...
  title: string;
  children?: React.ReactNode;
  primaryAction?: { label: string; onClick?: () => void; iconLeft?: React.ReactNode };
    ...
    >
      {icon && (
        <div className="mx-auto mb-4 flex h-[60px] w-[60px] items-center justify-center">
          <div className="[&>svg]:h-[60px] [&>svg]:w-[60px]">{icon}</div>
        </div>
      )}
...
export function InvitationSentCard({
  ...
  return showOverlay ? <ModalOverlay onClose={onClose}>{card}</ModalOverlay> : card;
}
...
export function RemoveUserCard({
  ...
  return showOverlay ? <ModalOverlay onClose={onCancel}>{card}</ModalOverlay> : card;
}
...
export function UserRemovedCard({
  ...
  const card = (
    <NotifCard
      ...
      title="User Removed"
    >
      <>Removed <span className="font-medium">{name}</span> successfully.</>
    </NotifCard>
  );
...
export function ResendInviteCard({
  ...
  const card = (
    <NotifCard
      title="Resend Invite"
      secondaryAction={{ label: "Cancel", onClick: onCancel }}
      primaryAction={{ label: "Resend Invite", onClick: onResend, iconLeft: PinkSendIcon }}
      pill
    >
      The staff member will receive an email with a temporary code and link to create their
      account.
    </NotifCard>
  );
